...
const MissingPet = () => {
  ...
  const fetchMarkers = async () => {
    try {
      const response = await axios.get('https://wildguard-back-2.onrender.com/api/location');
  ...
  const sendNearbyNotification = async (distance) => {
    try {
      const response = await axios.post('https://wildguard-back-2.onrender.com/api/notify', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingPetId) {
        await axios.put(`https://wildguard-back-2.onrender.com/api/pets/${editingPetId}`, petData);
        setEditingPetId(null);
      } else {
        await axios.post('https://wildguard-back-2.onrender.com/api/pets', petData);
      }
  ...
  const handleDelete = async (id, marker) => {
    try {
      await axios.delete(`https://wildguard-back-2.onrender.com/api/pets/${id}`);
