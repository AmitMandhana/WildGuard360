  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res?.success === false || !res.token) {
        ...
        dispatch(UserLogin({ token: res.token, userName: res.userName }));
      ...
      <div className='w-full md:w-4/3 h-fit lg:h-full 2xl:h-5/6 py-8 lg:py-0 flex bg-[#70ac54] rounded-xl overflow-hidden shadow-xl'>
        <div className='w-full lg:w-1/2 h-full p-10 2xl:px-20 flex flex-col justify-center'>
          <div className='w-full flex gap-2 items-center mb-6'>
            ...
            <span className='text-2xl text-[#20570E] font-semibold'>WildMedia360</span>
          </div>
          ...
          <form className='py-8 flex flex-col gap-5' onSubmit={handleSubmit(onSubmit)}>
            ...
            {errMsg && (
              <span aria-live="assertive" className={`text-sm ${errMsg ? "text-[#f64949fe]" : "text-[#2ba150fe]"} mt-0.5`}>
                {errMsg}
              </span>
            )}
          ...
          <p className='text-ascent-2 text-sm text-center'>
            ...
            <Link to='/ngoregister' className='text-[#065ad8] font-semibold ml-2 cursor-pointer'>Create Account</Link>
          </p>
        </div>
        ...
        <div className='hidden w-1/2 h-full lg:flex flex-col items-center justify-center bg-blue'>
          <div className='relative w-full flex items-center justify-center'>
            ...
            <div className='absolute flex items-center gap-1 bg-white left-160 top-10 py-2 px-5 rounded-full'>
              <BsShare size={14} /><span className='text-xs font-medium'>Share</span>
            </div>
            <div className='absolute flex items-center gap-1 bg-white right-40 top-6 py-2 px-5 rounded-full'>
              <ImConnection /><span className='text-xs font-medium'>Connect</span>
            </div>
            <div className='absolute flex items-center gap-1 bg-white left-12 bottom-6 py-2 px-5 rounded-full'>
              <AiOutlineInteraction /><span className='text-xs font-medium'>Interact</span>
            </div>
          </div>
